...
def analyse_cover_rate(df, group_col, group0, group1, feature_list = [], col_no_cover_dict={}):
    """ 
    计算两个人群中，各特征覆盖率与覆盖人数
    Parameters:
        df: DataFrame
    ...
    if not col_no_cover_dict:
        col_no_cover_dict = {'int64' :[-1],'float64': [-1.0],'str': ["-1", "unknown"],'object':["-1", "unknown"],'bool':[]}
    ...
    for col_name in df.columns:
        ...
        tot0.append(rownum0); cnt0.append(rownum0-no_cover0); ratio0.append(cover_ratio0)
        tot1.append(rownum1); cnt1.append(rownum1-no_cover1); ratio1.append(cover_ratio1)
        feat_type.append(df[col_name].dtype)
        
    res_df = pd.DataFrame(data = {'features':features, 
                                  '%s 总人数' % group0: tot0, '%s 覆盖人数' % group0 : cnt0, "%s 中覆盖人数占比" % group0 : ratio0,
                                  '%s 总人数' % group1: tot1, '%s 覆盖人数' % group1 : cnt1, "%s 中覆盖人数占比" % group1 : ratio1,
                                 'type': feat_type})
    return res_df



def analyse_01(group_df_col, group0, group1, feature_df_col, tag0, tag1, draw_pics = False, statistics = "chi2"): 
    """
    ...
    Notes:
        卡方检验中 H0：π1=π2，H1：π1≠π2，卡方检验自由度为 1，卡方值 >= 10.828，p-value <= 0.001 可以认为有显著差异。
    """    
    ...
    features = [tag0, tag1]
    g0 = groups.get_group(0); col0 = [g0[g0.feature == 0].shape[0], g0[g0.feature == 1].shape[0]] 
    g1 = groups.get_group(1); col1 = [g1[g1.feature == 0].shape[0], g1[g1.feature == 1].shape[0]]  
    ratio0 = [1.0 * col0[0] / (col0[0] + col0[1]), 1.0 * col0[1] / (col0[0] + col0[1])] if col0[0] + col0[1] > EPS else [None, None]
    ratio1 = [1.0 * col1[0] / (col1[0] + col1[1]), 1.0 * col1[1] / (col1[0] + col1[1])] if col1[0] + col1[1] > EPS else [None, None]
    res_df = pd.DataFrame(data = {'features': features, '%s'%group0: col0, '%s比率' % group0 : ratio0, '%s'%group1: col1, '%s比率'%group1: ratio1}) 
    
    # 绘制图像
    if draw_pics == True: 
        index = [group0, group1]
        df_barh1 = pd.DataFrame({tag0: [col0[0], col1[0]], tag1: [col0[1], col1[1]]}, index = index)
        df_barh1.plot.barh(stacked=True)
        df_barh2 = pd.DataFrame({tag0: [ratio0[0], ratio1[0]], tag1: [ratio0[1], ratio1[1]]}, index = index)
        df_barh2.plot.barh(stacked=True)
        
    # 计算 chi2
    if statistics == "chi2":
        a=col0[0]; b=col0[1]; c=col1[0]; d=col1[1]; n=a+b+c+d
...
def analyse_hist(group_df_col, group0, group1, feature_df_col, draw_pics = False, statistics = "t-test"):
    """
    ...
    groups = df.groupby('group')
    g0 = groups.get_group(0); vec0 = g0[g0.feature > - EPS]['feature']
    g1 = groups.get_group(1); vec1 = g1[g1.feature > - EPS]['feature']
    features = ['mean', 'std']
...
def analyse_pie(group_df_col, group0, group1, feature_df_col, col_no_cover_dict={}, draw_pics = False, statistics = "psi"):  
    """
    ...
    if not col_no_cover_dict:
        col_no_cover_dict = {'int64' :[0,-1],'float64': [-1.0],'str':["-1", "unknown"], 'object':["-1", "unknown"],'bool':[]}
    ...
    if draw_pics:
        ...
        for x in portion_type_list:
            portion_list.append(x[0]); type_list.append(x[1])
        df_draw = pd.DataFrame({"portion %s" % group0: portion_list}, index = type_list)
        ...
        for x in portion_type_list:
            portion_list.append(x[0]); type_list.append(x[1])
        df_draw = pd.DataFrame({"portion %s" % group1: portion_list}, index = type_list)
    ...
    if statistics == "psi":
        for item in psi_type_list:
            x = item[1]
            type_.append(x); ratio0.append(portion0[x]); ratio1.append(portion1[x]); psi_list.append(item[0])
        res_df = pd.DataFrame(data = {'type': type_, '%s 比例' % group0: ratio0, '%s 比例' % group1: ratio1, 'psi' : psi_list})
        return res_df, psi_sum
    
    for item in psi_type_list:
        x = item[1]
        type_.append(x); ratio0.append(portion0[x]); ratio1.append(portion1[x]);
    res_df = pd.DataFrame(data = {'type': type_, '%s 比例' % group0: ratio0, '%s 比例' % group1: ratio1})
    print("暂未支持此统计量查询")
    return res_df
...
if __name__ == '__main__':  
    ...
    #analyse_cover_rate(df, "basic_4", "ins", "water", feature_list = ['age', 'gender', 'work_city'])

    #df = pd.read_csv('prob.csv', header = 0)
    #print(analyse_hist(df['prob4'], 'ins', 'water', df['proba_sports'], draw_pics = True))

    #df = pd.read_csv('car.csv', header = 0)
    #print(analyse_01(df["car4"], "ins", "water", df["own_car"], "no car", "own car",  draw_pics = True, statistics = "chi2"))

    #df = pd.read_csv('basic.csv', header = 0)
    #print(analyse_pie(df["basic_4"], "ins", "water", df["age"], draw_pics = True))
