...
class DeepFM:
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            # first order term
            fm_first_order_bias = tf.nn.embedding_lookup(
                self.weights['bias_matrix'], self.feat_index)
            self.fm_first_order_vectors = tf.reduce_sum(
                tf.multiply(fm_first_order_bias, feat_value_tmp), 2)
    ...
    def _init_weights(self):
        ...
        weights['bias_matrix'] = tf.Variable(tf.random_normal(
            shape=[self.feature_size, self.embedding_size],
            mean=0.0, stddev=0.01),
            name='bias_matrix')
    ...
    def fit(self, x_train_index, x_train_value, y_train,
            x_valid_index=None, x_valid_value=None, y_valid=None,
            early_stopping=False,
            refit=False, refit_epochs=100, refit_verbose=0, stop_threshold=5):
