...
class Graph(defaultdict):
    ...
    def random_walk(self, path_length, alpha=0, rand=random.Random(), start=None):
      ...
      returns
      a truncated random walk.(返回一个固定长度的随机游走序列)
    """
        G = self
        if start:
            path = [start]
...
# 其中*的含义就是将其中所有元素取出来，然后将每个元素相应位置的分量合并
def grouper(n, iterable, padvalue=None):
    """
    grouper(3, 'abcdefg', 'x') --> ('a','b','c'), ('d','e','f'), ('g','x','x')
    """
    # 迭代器的数量n == 每个worker处理的随机游走的数量
...
def load_adjacencylist(input_path, undirected=False, chunksize=10000, unchecked=True):

    # 如果设为未校验模式，那么配置相应的解析函数以及转换函数
    if unchecked:
    ...
    t1 = time()
    logger.info('将边转成图 共用时间 {}s'.format(t1 - t0))
